...
const UserLogForm = ({
  ...
  const handleSubmit = (e) => {
    ...
    const updatedUserLog = {
      work_out_date: userLog.work_out_date,
      work_out_id: userLog.work_out_id,
      weight,
      reps,
      notes,
    };
    if (logExists) {
      axios.put(`/api/user_logs/${userLog.id}`, updatedUserLog).then((res) => {
  ...
  return (
    <Segment>
      ...
      {!showLogForm && <Card.Meta>{notes}</Card.Meta>}
      {/* <Card.Desexion></Card.Description> */}
      {showLogForm && (
