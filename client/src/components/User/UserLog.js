...
const UserLog = ({ workoutDate, exerciseId, workoutId }) => {
  ...
  useEffect(() => {
    axios
      .get(`/api/user_logs/${encodeURIComponent(workoutDate)}`, {
        params: { work_out_id: workoutId }
      })
      .then(res => {
        setUserLog([...res.data]);
      })
  ...
  const generateBlankUserLog = e => {
    setUserLog([
      ...userLog,
      {
        reps: null,
        weight: null,
        work_out_id: workoutId,
        work_out_date: workoutDate
      }
    ]);
    setShowLogForm(true);
  };
  ...
  const duplicateLastRound = () => {
    ...
    const updatedUserLog = {
      work_out_date: userLog[last].work_out_date,
      work_out_id: userLog[last].work_out_id,
      weight: userLog[last].weight,
      reps: userLog[last].reps,
      notes: userLog[last].notes
    };
    axios
      .post(`/api/user_logs`, updatedUserLog)
      .then(res => {
