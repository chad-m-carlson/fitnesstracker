...
const ExerciseForm = ({
  ...
  useEffect(() => {
    ...
    if (exercise) {
      setEditing(true);
      axios
        .get(`/api/exercise_categories_exercises/${exercise.id}`)
        .then(res =>
          setSelectedExerciseCategories([...res.data.map(d => d.id)])
        );
  ...
  const handleSubmit = e => {
    ...
    if (!editing) {
      ...
      axios
        .post(`/api/exercises`, exerciseToSave)
        .then(res => {
      ...
      axios.put(`/api/exercises/${exercise.id}`, exerciseToSave).then(res => {
        setShowExerciseForm(!showExerciseForm);
        setExerciseChanged(!exerciseChanged);
  ...
  const handleCheckbox = id => {
    if (selectedExerciseCategories.includes(id)) {
      const filteredExerciseCategories = selectedExerciseCategories.filter(
        e => e !== id
      );
  ...
  return (
    <Segment basic>
      <Header as="h1" textAlign="center">
        {editing ? `Edit ${exercise.name}` : "Add Exercise"}
      </Header>
