...
class NewWorkOutForm extends React.Component {
  ...
  componentDidMount = () => {
    if (this.props.editing) {
      this.setState({
        showReps: true,
        repAmount: this.props.repAmount,
        repPace: this.props.repPace,
        notes: this.props.exercise.notes,
        exercise_order: this.props.exercise.exercise_order,
      });
      ...
      this.setState({
        ...
        repPace: this.props.repPace,
        notes: this.props.exercise.notes,
        exercise_order: this.props.index,
      });
    } else {
      axios
        .get(
          `/api/work_outs/${encodeURIComponent(
            getSimpleDate(this.props.date)
          )}/exercises/${this.props.exercise.id}`
        )
        .then((res) => {
          this.setState({ workout: [...res.data] });
          res.data.map((w) => {
            if (this.props.exercise.id === w.exercise_id)
              this.setState({ repAmount: w.rep_amount, repPace: w.rep_pace });
          });
        });
    }
  };
  ...
  handleSubmit = (e) => {
    ...
    const { repAmount, repPace, notes, exercise_order, includeSuperset } =
      this.state;
  ...
  generateRepsDropdown = () => {
    const repAmount = [];
    const repPace = [];
    this.props.reps.amount.map((r) =>
      repAmount.push({ text: r.amount, value: r.amount })
    );
    this.props.reps.pace.map((r) =>
      repPace.push({ text: r.pace, value: r.pace })
    );
          ...
          <div style={{ display: "flex" }}>
            {!this.state.showRepAmountInput && (
              <Form.Field
                ...
                value={
                  this.state.workout ? this.state.repAmount : repAmount.value
                }
            ...
            <DivButton
              onClick={() =>
                this.setState({
                  showRepAmountInput: !this.state.showRepAmountInput,
                })
              }
            >
              Other?
        ...
        <DropdownContainer>
          <div style={{ display: "flex" }}>
            ...
            <DivButton
              onClick={() =>
                this.setState({
                  showRepPaceInput: !this.state.showRepPaceInput,
                })
              }
            >
              Other?
  ...
  render() {
    return (
      <>
        {!this.state.showReps && (
          <ExerciseContainer
            onClick={() => this.setState({ showReps: !this.state.showReps })}
          >
              ...
              <>
                ...
                <Form.Group widths="equal">
                  ...
                  <Form.Input
                    ...
                    onChange={(e) =>
                      this.setState({ exercise_order: e.target.value })
                    }
                  />
                  <Form.Checkbox
                    ...
                    onChange={(e) =>
                      this.setState({
                        includeSuperset: !this.state.includeSuperset,
                      })
                    }
                  />
                </Form.Group>
                ...
                >
                  ...
                  <Button size="tiny">
                    {this.props.editing ? "Save Changes" : "Add To Workout"}
                  </Button>
                </div>
              </>
            </Form>
